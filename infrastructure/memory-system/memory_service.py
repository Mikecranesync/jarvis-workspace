#!/usr/bin/env python3
"""
Jarvis Memory System - ChromaDB Implementation
Replaces broken Gemini embeddings with local sentence-transformers
"""
import asyncio
import hashlib
import logging
...
class MemoryFileHandler(FileSystemEventHandler):
    ...
    def _schedule_reindex(self):
        ...
        self.debounce_timer = asyncio.get_event_loop().call_later(
            ...
            lambda: asyncio.create_task(self.memory_service.index_memory_files())
        )

class MemoryService:
    """ChromaDB-based memory service for Jarvis"""
    ...
    async def initialize(self):
        ...
        try:
            ...
            # Initialize sentence transformer model for dimensions info
            logger.info(f"Loading sentence transformer model: {EMBEDDING_MODEL}")
            self.model = SentenceTransformer(EMBEDDING_MODEL)
            
            logger.info(f"Memory service initialized successfully!")
...
app = FastAPI(
    ...
    description="ChromaDB-based semantic memory search for Jarvis",
    version="1.0.0"
)
...
if __name__ == "__main__":
    # Run the service
    logger.info(f"Starting Jarvis Memory System on {SERVICE_HOST}:{SERVICE_PORT}")
    uvicorn.run(
        app, 
        host=SERVICE_HOST, 
        port=SERVICE_PORT,
