...
## Photo → CMMS Bot Built & Launched
- **Built `photo_to_cmms_bot.py`** at `C:\Users\hharp\clawd\projects\rivet-pro\photo_to_cmms_bot.py`
- Flow: Telegram photo → Gemini 2.5 Flash vision analysis → Atlas CMMS asset creation → work order creation → formatted reply
- **Bot token:** 7855741814 (@testbotrivet_bot) — separate from Clawdbot's token (8519329029)
- **CMMS:** Atlas CMMS running on VPS at `http://72.60.175.144:8080` (Docker container `atlas-cmms`)
  - Auth: POST /auth/signin with `{email, password, type:"client"}` → accessToken
  - Password: `Bo1ws2er@12` (the `@` is required — `Bo1ws2er12` without it fails)
- **Gemini:** Using deprecated `google.generativeai` package — should migrate to `google.genai` eventually
- Bot launched successfully as background process (session: lucky-daisy, PID 6148)
- Mike's goal: demo for his wife in 30 minutes — photo of equipment → auto work order

## Infrastructure Notes
- **No Docker Desktop locally** — Docker commands fail on Mike's laptop
- **No CMMS locally** (localhost:8081 refused) — all CMMS is on VPS
- VPS also runs: Redis (6379), PostgreSQL/pgvector (5432), Atlas frontend (3000)
- Clawdbot gateway PID 31164, model claude-opus-4-5

## CMMS Password Reset
- Old password: `Bo1ws2er@12` → **DEAD, no longer works**
...
- `POST /auth/signin` → `{accessToken}`
- `POST /assets` → create asset (payload: `{name, description, model, area, status}`)
- `GET /assets/mini` → list all assets (mini format)
- `POST /assets/search` → search assets (paginated: `{filterFields, pageNum, pageSize}`)
- `POST /work-orders` → create work order (payload: `{title, description, priority, status, category, asset:{id}}`)
- `POST /work-orders/search` → search WOs (paginated)
- **NOTE:** `GET /assets` returns 405! Use `/assets/mini` or `POST /assets/search`

## Production Hardening (05:20 session)

### What was wrong
- Bot ran on Mike's laptop → dies when laptop sleeps
- VPS had old `rivet-pro-bot.service` polling same token → 409 Conflict
- Hardcoded credentials in source
- Wrong CMMS list endpoints (GET /assets → 405)
- No dedup → created duplicate Micro820 assets (#147, #149)
...
1. **Full production rewrite** of `photo_to_cmms_bot.py`:
   ...
   - On Conflict error: hard exit → systemd restarts clean
...
- Fix: removed `category` and `status` from WO payload — both default server-side
- Verified: WOs now create successfully (tested with Nextys PSU → WO created + asset #150)

### CMMS Frontend Login Broken — FIXED
- **Root cause 1:** `runtime-env.js` had `API_URL: "https://cmms.maintnpc.com"` — dead domain
...
- Login tested live via browser automation: ✅ works

### Watchdog Fix
- systemd `WatchdogSec=120` was killing bot every 2 min (Python doesn't send sd_notify)
- Removed WatchdogSec from service file — bot now runs indefinitely
...
- Bot responses now include clickable links:
  - 🔗 View Work Order → `http://72.60.175.144/app/work-orders/{id}`
  - 🔗 View Asset → `http://72.60.175.144/app/assets/{id}`
...
1. **Registration page** at `http://72.60.175.144/register`
   ...
   - Multi-step: form → OTP verification → success
...
3. **Twilio Verify API** for phone OTP
   ...
   - Auth Token: 8cc6d286bac2fe033df7d49eb957efbc (updated — old one was expired)
...
4. **SMS drip campaign** — welcome + Day 2 + Day 5 messages (background worker)
5. **Caddy routes** — registration routes placed BEFORE generic `/api/*` to avoid CMMS backend catching them

### Bot Freemium Gate
- Removed `ALLOWED_USERS` allowlist — bot open to ALL Telegram users
- 3 free photo analyses per user (tracked in SQLite `telegram_usage` table)
- After 3: registration wall with inline button → `http://72.60.175.144/register?tg={id}`
...
- Phone: +18636517679
- Hit "custom_code not allowed" error → fixed by using Twilio's built-in OTP
...
### Git Commits
- `12be813` — Photo→CMMS bot + deploy configs
- `33375c9` — Freemium registration system
- `769e902` — Freemium gate on bot

## KB Harvester (08:30 EST — In Progress)
...
- **Bot:** Running on VPS as `plc-copilot.service` — freemium mode (3 free, then registration)
...
## RideView Web Tool — Major Overhaul (13:00-14:10 EST)

### Problem Identified
- HSV-only torque stripe detection was **completely failing** — detecting Coca-Cola cans, multimeters, push button panel colors as "stripes"
- Algorithm had no concept of WHERE a bolt is — just found any orange/red/yellow in entire frame
- Results: "Stripes: 14, Continuity: 0.00" on a photo with 2 bolts — useless for a tech
...
- **Rebuilt `analyzer.py`** — now uses Gemini 2.0 Flash Vision API as primary classifier
...
### Video Support Added
- New "Record Video" button (🎬) opens native camera in video mode
- Video uploaded → frames extracted every 1 second (max 20 frames) → each analyzed by Gemini
...
### Files Changed
- `C:\Users\hharp\clawd\projects\rideview\web_tool\analyzer.py` — complete rewrite (Gemini Vision + HSV fallback + video frame extraction)
- `C:\Users\hharp\clawd\projects\rideview\web_tool\app.py` — video upload handling, async analysis
- `C:\Users\hharp\clawd\projects\rideview\web_tool\templates\index.html` — video UI, rich metadata display

### Deployment
- Deployed to VPS: `/opt/rideview/` (analyzer.py, app.py, templates/index.html)
- Added `GEMINI_API_KEY` env var to `rideview.service`
- Service running: `systemctl restart rideview` → active
- URL: https://72-60-175-144.sslip.io/inspect
- `aiohttp` installed on VPS for async Gemini API calls

### Technical Notes
- Gemini API key hardcoded as fallback in analyzer.py (same key as Photo→CMMS bot)
...
## Key Lessons
- Docker `runtime-env-cra` generates `runtime-env.js` from env vars — use `-e` flags, not volume mounts
...
- systemd WatchdogSec requires sd_notify from the app — don't use it with simple Python scripts
- Always test frontend login end-to-end (browser), not just API — CORS, runtime config, caching all matter
- **HSV color detection is useless without ROI isolation** — any colored object in frame triggers false positives. Use vision AI for semantic understanding, HSV only for visual overlay within known regions.
...
- **Problem:** Gemini's bbox estimates were inaccurate — boxes landed above/left of actual bolt
...
- No more Gemini bbox guessing — whole cropped image IS the bolt region
- HSV glow runs on full cropped image

### Iteration 3: Overlay cleanup
- Text overlay covered entire photo — removed it (details shown in web UI below image)
- Glow was "spaghetti" — too many tiny contours. Added min area filter (0.1% of image)
...
- Picker API requires interactive browser selection — not suitable for bulk pull
- **Pivoted to Google Takeout** — Mike initiated export, waiting for email
...
- Sub-agent wrote WHITEPAPER.md (~35KB) — full R&D paper on the hybrid VLM approach
...
- Market research sub-agent: competitors, pricing, grants, patents, go-to-market → MARKET_RESEARCH.md
- Technical research sub-agent: on-device ML, camera hardware, CV pipeline, offline arch → TECHNICAL_RESEARCH.md
- Both completed and saved to RideView repo

## Proactive Operations Setup (15:55-16:46 EST)

### Watched TWiST E2240 — Clawdbot episode
...
### PRD Created
- PROACTIVE_OPS_PRD.md — full plan for Jarvis as AI Chief of Staff
...
- 7:00 PM EST: Signal Hunter — autonomous web/Reddit/grant scan (isolated agent)
...
- Reason: testing app at Universal Orlando at night — employer may not like it
- **Identity:** jarvis@factorylm.com
- **Domain:** factorylm.com — Mike tried Namecheap but locked out (password reset emails not arriving)
...
## FactoryLM Goes Live (19:28-19:40 EST)

### Domain + SSL
- factorylm.com DNS pointed to VPS (72.60.175.144) via Namecheap
- Caddy auto-provisioned Let's Encrypt SSL certificate
- All services routed through factorylm.com

### Production Landing Page
- Full SaaS landing: hero, features (3 products), how-it-works, pricing tiers, signup form
- Pricing: Free Beta / Pro $99/mo / Enterprise Custom
- Mobile-responsive, dark theme, scroll animations
- "Try Inspector" links to /inspect
- Contact: mike@cranesync.com

### Signup API (FastAPI + SQLite)
- POST /api/signup — collects name, email, company, role
- Telegram notification on every signup (instant alert to Mike)
- Duplicate detection, email validation
- Admin endpoint: GET /api/signups (API key protected)
- Service: factorylm-api.service on port 8090

### Full Rebrand
- ALL "RideView" and "Maint-NPC" references removed from public-facing code
- Everything is now "FactoryLM" — landing page, inspect tool, PWA manifest
- Committed + pushed to GitHub

### Services Running on VPS
1. factorylm-api (port 8090) — signup backend
2. rideview (port 8002) — inspection tool (branded as FactoryLM)
3. trello-webhook (port 8078) — @jarvis Trello → Telegram
4. caddy (ports 80/443) — reverse proxy + auto SSL
5. atlas-cmms (ports 3000/8080) — CMMS frontend + backend

### First Signup
- Test signup #1 registered successfully
- Telegram notification delivered to Mike

## Mike's Schedule (IMPORTANT)
- **Third shift worker** — wakes up ~3:00 PM EST, goes to bed ~10:00 AM EST
